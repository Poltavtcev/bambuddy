...
export function ScheduleOptionsPanel({
  ...
  return (
    <div className="space-y-4">
      {/* Schedule type */}
      <div>
        <label className="block text-sm text-bambu-gray mb-2">{t('printModal.scheduleWhen')}</label>
      ...
      {options.scheduleType === 'scheduled' && (
        <div>
          <label className="block text-sm text-bambu-gray mb-1">{t('printModal.scheduleDateTime')}</label>
          <div className="flex gap-2">
            {/* Date input */}
          ...
          {(!isDateValid || !isTimeValid) && (
            <p className="mt-1 text-xs text-red-400">
              {t('printModal.scheduleInvalidDateTime')}
            </p>
          )}
        </div>
      )}
      ...
      <div className="flex items-center gap-2">
        ...
        <label htmlFor="autoOffAfter" className={`text-sm flex items-center gap-1 ${canControlPrinter ? 'text-bambu-gray' : 'text-bambu-gray/50'}`}>
          ...
          {t('printModal.autoPowerOffAfter')}
        </label>
      </div>
      ...
      <p className="text-xs text-bambu-gray">
        {options.scheduleType === 'asap'
          ? t('printModal.scheduleHelpAsap')
          : options.scheduleType === 'scheduled'
          ? t('printModal.scheduleHelpScheduled')
          : t('printModal.scheduleHelpManual')}
      </p>
    </div>
  );
}
